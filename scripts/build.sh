...
if [[ ${CMD} = "build" ]]; then
    if [[ -n ${GOOS} && -n ${GOARCH} ]]; then
        GOOS=${GOOS} GOARCH=${GOARCH} go build -o ${OUT} -ldflags "${FLAGS}" -v github.com/Joshcarp/sysl/cmd/sysl
    else
        go build -o ${OUT} -ldflags "${FLAGS}" -v github.com/Joshcarp/sysl/cmd/sysl
    fi
